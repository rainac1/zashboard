...
// 维护成活跃连接表,并产出驱动层统一的 ConnectionsPayload。
// 瞬时速率由 assembly/connections 的门面按访问器 diff 求得,这里只负责活跃表与
// 「本拍新关闭」增量。
...
import type { ConnectionAccessor, ConnectionsPayload, Stream } from '../driver/types'
...
export const connectionAccessor: ConnectionAccessor = {
  ...
  // sing-box 没有 clash 那套 chains/type 推断,最终出站就是 outboundType。
...
export const subscribeConnections = (): Stream<ConnectionsPayload> => {
  ...
  const close = (id: string, base?: PbConnection | Connection) => {
    ...
    conns.delete(id)
    if (c) newlyClosed.push(c as Connection)
  }
      ...
      switch (event.type) {
        case ConnectionEventType.CONNECTION_EVENT_NEW:
          ...
          if (event.connection) {
            ...
            else conns.set(event.id, event.connection as Connection)
          }
          break
        case ConnectionEventType.CONNECTION_EVENT_UPDATE: {
          if (event.connection) {
            ...
            // 仅 delta:沿用上次的连接,累加总量 —— 门面按访问器 diff 求瞬时速率。
